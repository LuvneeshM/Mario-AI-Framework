...
public class Mario extends MarioSprite {
	...
	public void update() {
		...
		if (actions[MarioActions.JUMP.getValue()] || (jumpTime < 0 && !onGround)) {
			...
			if(jumpTime == 0) { //max jump
				if(endJump == 1) { //record at the max, don't include the falling
					endJump = 2;
					this.world.addEvent(EventType.JUMP, -1);
				}
				
			}
		} else {
			if(endJump == 1) { //user ended the jump early, record from the height
				endJump = 2;
				this.world.addEvent(EventType.JUMP, -1);
			}
			jumpTime = 0;
